    ...
    def sort(self) -> None:
        ...
        for n in range(len(self.lista)-1, 0, -1):
            for i in range(n):
                ...
                title = st.title(
                    f"{list(self.lista[i].keys())} > {list(self.lista[i + 1].keys())} = {list(self.lista[i].keys()) > list(self.lista[i + 1].keys())}", anchor=None)

                # Controla o tempo de uma execução pra outra
                time.sleep(self.velocity)

                title.empty()  # Limpe os elementos do título
                graphic.empty()  # Limpe os elementos do gráfico

                # Pegando apenas a key do dict, para fazer a comparação
                if list(self.lista[i].keys()) > list(self.lista[i + 1].keys()):

                    troca = True
                    self.lista[i], self.lista[i +
                                              1] = self.lista[i + 1], self.lista[i]
